      ...
      <div className="container-x relative grid items-center gap-10 py-20 sm:py-28 lg:grid-cols-12">
        <div className="lg:col-span-7">
          ...
          <p className="mt-5 max-w-2xl text-lg text-white/85">
            ...
            render and driveway back to life — LayMac Solutions is your
        ...
        <div className="lg:col-span-5">
          <div className="relative">
            ...
            <div className="absolute -bottom-5 -left-5 hidden rounded-2xl bg-sand p-4 shadow-soft sm:block">
              <div className="flex items-center gap-1 text-coast">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Icon key={i} name="star" className="h-4 w-4" />
                ))}
              </div>
              <p className="mt-1 text-sm font-semibold text-cream">5.0 from local reviews</p>
              <p className="text-xs text-cream/60">Honest pricing. Tidy work.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container-x section">
        <div className="grid gap-12 lg:grid-cols-12">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:col-span-8">
            {points.map((p) => (
              <div key={p.title} className="rounded-2xl bg-navy p-6 ring-1 ring-cream/10">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-coast text-white">
                  <Icon name="check" className="h-5 w-5" />
                </div>
                <h3 className="mt-4 text-lg font-semibold">{p.title}</h3>
                <p className="mt-2 text-sm text-cream/75">{p.body}</p>
              </div>
            ))}
          </div>
        </div>
    ...
    <section className="bg-navy text-white">
      <div className="container-x section">
        ...
        <ol className="mt-12 grid gap-6 md:grid-cols-3">
          {steps.map((s) => (
            <li key={s.n} className="rounded-2xl bg-navy/5 p-7 ring-1 ring-white/10">
      ...
      <div className="container-x section">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {[
            ...
            <div
              key={img.src}
              className="overflow-hidden rounded-2xl bg-navy shadow-soft ring-1 ring-cream/10"
            >
              <Image
                src={img.src}
                alt={img.alt}
    ...
    <section className="bg-sand">
      <div className="container-x section grid gap-10 lg:grid-cols-2">
        <div>
          ...
          <ul className="mt-6 flex flex-wrap gap-2">
            {site.serviceAreas.map((s) => (
              <li
                key={s}
                className="rounded-full bg-navy px-3 py-1 text-sm text-cream ring-1 ring-cream/15"
              >
                {s}
              </li>
            ))}
        ...
        <div className="overflow-hidden rounded-2xl ring-1 ring-cream/10">
          <iframe
            title="LayMac Solutions service area"
