...
export const metadata: Metadata = {
  ...
  description:
    "Before and after photos of recent painting, soft-wash, pressure-cleaning and lawn jobs by LayMac Solutions."
};
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {galleryItems.map((item) => (
              <article key={item.title} className="card overflow-hidden p-0">
                <div className="relative aspect-[4/5] w-full overflow-hidden bg-cream/5">
                  <Image
                    src={item.image}
                    alt={item.title}
                    fill
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between">
                    <span className="rounded-full bg-cream/5 px-2.5 py-1 text-xs font-medium text-cream">
                      {item.category}
                    </span>
                  </div>
                  <h3 className="mt-2 text-base font-semibold text-cream">
                    {item.title}
                  </h3>
